# Claude Code Project Instructions — cpal-kotlin

## Project Overview

This is **cpal-kotlin**, a clean-room Kotlin Multiplatform port of the upstream Rust crate [`cpal`](https://crates.io/crates/cpal).

The upstream Rust source is the read-only translation oracle. When porting begins, clone it into `tmp/cpal/` (gitignored). **Never edit `tmp/`.** If upstream looks wrong, the bug is in the port or in your understanding of Rust, not in `tmp/`.
...
```kotlin
// port-lint: source <path-relative-to-tmp/cpal>
